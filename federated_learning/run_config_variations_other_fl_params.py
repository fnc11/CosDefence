...
def run_sequentially(summary_data_list, config, random_dists):
    ...
    local_epochs_var = [3, 7, 10]
    lr_vars = [0.0001, 0.001, 0.01, 0.1]
    fed_rounds_var = {'mnist':[25, 30], 'fmnist': [50, 60], 'cifar10': [100, 200]}

    for fed_rounds in fed_rounds_var[config['DATASET']]:
        config['FED_ROUNDS'] = fed_rounds
        try:
            summary_data_list.append(run_and_summarize(config, random_dists))
        except Exception as e:
            print(e)

    for local_epochs in local_epochs_var:
